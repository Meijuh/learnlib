...
public abstract class Lasso<S, A extends SimpleDTS<S, I> & Output<I, D>, I, D>
    ...
    /**
     * The finite representation of the lasso.
     */
    @Getter
    private final Word<I> word;

    /**
     * The loop of the lasso.
     */
    @Getter
    private final Word<I> loop;

    /**
     * The prefix of the lasso.
     */
    @Getter
    private final Word<I> prefix;

    /**
     * The finite representation of the output of the lasso.
     */
    @Getter
    ...
    /**
     * The input alphabet of this automaton.
     */
     ...
     * @param unfoldTimes the number of times the loop needs to be unrolled, must be > 0.
     */
    public Lasso(A automaton, Collection<? extends I> inputs, int unfoldTimes) {
        ...
        do {
            ...
            I input = null;
            for (int j = 0; j < inputAlphabet.size() && input == null; j++) {
                final Word<I> word = wb.toWord().append(inputAlphabet.getSymbol(j));
                final S state = automaton.getSuccessor(automaton.getInitialState(), word);
                if (state != null) input = inputAlphabet.getSymbol(j);
            }

            // check whether we actually found such an input
            if (input == null) throw new IllegalArgumentException(NO_LASSO);

            // append the input to the finite representation
            wb.append(input);

            // continue with the next state.
            current = automaton.getSuccessor(automaton.getInitialState(), wb.toWord());
        ...
        for (int u = 1; u < unfoldTimes; u++) wb.append(loop);
        ...
        for (int l = prefix.length(); l < word.length(); l += loop.length()) loopBeginIndices.add(l);
    }
    ...
    public Integer getSuccessor(Integer state, @Nullable I input) {
        ...
        if (state < word.length() && input.equals(word.getSymbol(state))) result = state + 1;
        else result = null;
    ...
    public Collection<Integer> getStates() {
        final Collection<Integer> states = IntStream.range(0, word.length() - 1).boxed().collect(Collectors.toSet());
        return states;
    }
    ...
    public static class MealyLasso<S, I, O> extends Lasso<S, MealyMachine<S, I, ?, O>, I, Word<O>>
        ...
        public Word<O> computeOutput(Iterable<? extends I> input) {
            ...
            final Word<O> result;
            if (output.equals(getOutput())) result = output;
            else result = Word.epsilon();
