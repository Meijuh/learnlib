 ...
 */
public abstract class UnfoldingModelChecker<I,
    ...
    /**
     * Constructs a new UnfoldingModelChecker.
     *
     ...
     */
    protected UnfoldingModelChecker(int minimumUnfolds, double multiplier) throws IllegalArgumentException {
    ...
    protected final int computeUnfolds(int size) {
        final int relativeUnfolds = new Double(Math.ceil(size * multiplier)).intValue();
    ...
    public void setMinimumUnfolds(int minimumUnfolds) throws IllegalArgumentException {
        if (minimumUnfolds < 1) throw new IllegalArgumentException("must unfold at least once");
        this.minimumUnfolds = minimumUnfolds;
    }

    @Override
    public void setMultiplier(double multiplier) throws IllegalArgumentException {
        if (multiplier < 0.0) throw new IllegalArgumentException("multiplier must be >= 0.0");
