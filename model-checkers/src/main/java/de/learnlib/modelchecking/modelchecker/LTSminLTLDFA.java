...
import net.automatalib.parser.FSM2DFAParser;
...
public class LTSminLTLDFA<I>
        extends LTSminLTL<I, DFA<?, I>, DFALasso<?, I>>
    ...
    private <S> void dfa2ETF(DFA<S, I> dfa, Collection<? extends I> inputs, File etf) throws IOException {
        ...
        if (!DFAs.isPrefixClosed(dfa, alphabet)) throw new ModelCheckingException("DFA is not prefix closed.");

        // remove all rejecting states
        final MutableDFA copy = new CompactDFA(alphabet, dfa.size());
     ...
     *
     * @see LTSminLTL#fsm2Lasso(File, SimpleDTS).
     */
    @Override
    protected DFALasso<?, I> fsm2Lasso(File fsm, DFA<?, I> hypothesis) throws IOException, FSMParseException {
        CompactDFA dfa = FSM2DFAParser.parse(fsm, getString2Input(), LABEL_NAME, LABEL_VALUE);
