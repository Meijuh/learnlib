...
public class SimulatorOmegaOracle<S, I, D, SO extends SimulatorOracle<I, D>>
    ...
    /**
     * The {@link SimulatorOracle} used to answer {@link de.learnlib.api.query.Query}s.
     */
    @Getter
     ...
     *
     * @see de.learnlib.api.oracle.OmegaQueryAnswerer#answerQuery(Word, Word, Set).
     */
    @Override
    public Pair<D, List<S>> answerQuery(Word<I> prefix, Word<I> suffix, Set<Integer> indices) {
        final List<S> states = new ArrayList();

        for (int i : indices) states.add(simpleDTS.getState(prefix.concat(suffix).prefix(i)));
